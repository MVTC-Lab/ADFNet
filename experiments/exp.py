...
class Exp(Exp_Basic):
    def __init__(self, args):
        ...
        self.best_val_loss = None # 新增属性记录最佳验证损失
        self.best_val_mae = float('inf')  # 新增：记录最佳MAE
        self.current_val_mae = None      # 新增：当前epoch的MAE
    def _build_model(self):
        ...
        model = SCINet(
            ...
            fft_ratio = self.args.fft_ratio,
            use_fft = self.args.use_fft,
            fft_mode=self.args.fft_mode,
            middle_layer_count=self.args.middle_layer_count,
            activation=self.args.activation,
            dropout1d_v=self.args.dropout1d_v,
    ...
    def valid(self, valid_data, valid_loader, criterion):
        ...
        if self.args.stacks == 1:
            ...
            self.current_val_mae = mae  # 新增：记录当前MAE
        ...
        print(f"Validation MAE: {mae:.4f}")  # 可选：输出MAE
        return total_loss

    def train(self, setting, return_best_val_mae=False):
        ...
        if not os.path.exists(path):
            os.makedirs(path)
        writer = SummaryWriter('event/run_ETT/{}'.format(self.args.model_name))
        ...
        for epoch in range(epoch_start, self.args.train_epochs):
            ...
            writer.add_scalar('test_loss', test_loss, global_step=epoch)
             ...
             # 更新最佳MAE
            if self.current_val_mae < self.best_val_mae:
                self.best_val_mae = self.current_val_mae
        ...
        log_content = (
            ...
            f"Model Config: data={self.args.data}, seq_len={self.args.seq_len}, pred_len={self.args.pred_len},\n" 
            f"batch_size={self.args.batch_size}, patience={self.args.patience}, lr={self.args.lr}, lradj={self.args.lradj},\n"
            f"hidden_size={self.args.hidden_size}, kernel={self.args.kernel},\n" 
            f"dilation={self.args.dilation}, window_size={self.args.window_size}, dropout={self.args.dropout},\n"
            f"positionalEcoding={self.args.positionalEcoding}, RIN={self.args.RIN},\n" 
            f"groups={self.args.groups}, levels={self.args.levels}, stacks={self.args.stacks}, num_decoder_layer={self.args.num_decoder_layer},\n" 
           
            f"fft_ratio={self.args.fft_ratio}, use_fft={self.args.use_fft}, fft_mode={self.args.fft_mode}\n"
            f"middle_layer_count={self.args.middle_layer_count}, activation={self.args.activation}, dropout1d_v={self.args.dropout1d_v},\n"
        ...
        # 在训练结束后记录最佳损失
        self.best_val_loss = early_stopping.val_loss_min
        print(f"self.best_val_loss:{self.best_val_loss}")

        if return_best_val_mae:
            print(f"self.best_val_mae:{self.best_val_mae}")
